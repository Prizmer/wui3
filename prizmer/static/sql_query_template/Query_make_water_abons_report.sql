...
CREATE OR REPLACE VIEW water_abons_report AS 
 WITH korp AS (
         SELECT objects_1.name,
          ...
          WHERE objects_1.name::text ~~ '%Корпус%Вода%'::text
        )
 SELECT korp.name,
    abonents.account_2,
