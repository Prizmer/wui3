   ...
   FROM (( SELECT abonents.guid AS ab_guid,
		  ...
		  types_meters.name as type_meter
           FROM objects objects1,
            ...
            resources,
		  public.types_meters
          WHERE (((objects.guid_parent)::text = (objects1.guid)::text) 
				 AND ((abonents.guid_objects)::text = (objects.guid)::text) 
				...
				and meters.guid_types_meters = types_meters.guid)
          GROUP BY types_meters.name, abonents.account_1, objects1.name, abonents.guid, abonents.name, objects.name, meters.factory_number_manual, resources.name, link_abonents_taken_params.coefficient, link_abonents_taken_params.coefficient_2, link_abonents_taken_params.coefficient_3
