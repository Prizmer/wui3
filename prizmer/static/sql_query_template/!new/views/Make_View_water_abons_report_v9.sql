-- View: public.water_abons_report
 ...
 SELECT korp.name,
    ...
    abonents.name AS ab_name,
        CASE
			...
			WHEN ((abonents.name)::text ~~ '%ХВС%'::text) THEN 'Холодное водоснабжение'::text
            ELSE 'error'::text
        END AS type_energo,
  ...
  WHERE (((params.guid)::text = (taken_params.guid_params)::text) AND 
		 ...
		 AND ((abonents.name not like 'ТЕПЛО%') AND (abonents.name not like '%Тепло%') AND (abonents.name not like '%тепло%')))
