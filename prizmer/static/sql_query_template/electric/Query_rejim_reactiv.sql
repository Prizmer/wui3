...
WHERE 
  ...
  various_values.date = ('01.'::text||(extract(month from timestamp '02.10.2017'))::text||'.'||(extract(year from timestamp '02.10.2017'))::text)::timestamp AND 
...
WHERE 
  ...
  various_values.id_taken_params = taken_params.id AND
  params.guid_names_params = names_params.guid AND
  various_values.date = '02.10.2017' AND 
  names_params.name = 'R+ Профиль' and
  abonents.name='Эл.щит. ГРЩ 31632596'
