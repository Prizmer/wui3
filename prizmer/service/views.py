...
def check_objects_and_abonents_structure(parent, obj, abon):
    dt = []
    cursor = connection.cursor()
    sQuery = """
    WITH parent_obj AS (
      ...
      FROM 
        public.objects
      JOIN        parent_obj ON parent_obj.obj_guid = objects.guid_parent
      JOIN         public.abonents ON abonents.guid_objects = objects.guid
      
	  WHERE parent_obj.obj_name = '%s'
	  AND objects.name = '%s'
	  AND  abonents.name = '%s'"""%(parent, obj, abon)
    cursor.execute(sQuery)
    dt=cursor.fatchall()
    return dt
...
#         
#         # dt = check_objects_and_abonents_structure(obj_l1, obj_l2, abon)
#         # print(dt)
#         # isNewObj_l1 = True
#         # if len(dt) > 0:
#         #      isNewObj_l1 = False  
#                   
#         # dt = check_objects_and_abonents_structure(obj_l2, obj_l3, abon)
#         # print(dt)
#         # isNewObj_l2 = True
#         # if len(dt) > 0:
#         #      isNewObj_l2 = False       
...
def get_or_create_object_with_parent(name, level, parent=None):
    """
    Получает или создает объект с учетом родителя.
    """
    if parent:
        try:
            obj = Objects.objects.get(name=name, guid_parent=parent)
        except Objects.DoesNotExist:
            obj = Objects(name=name, level=level, guid_parent=parent)
            obj.save()
    else:
        try:
            obj = Objects.objects.get(name=name, level=level)
        except Objects.DoesNotExist:
            obj = Objects(name=name, level=level)
            obj.save()
    return obj

def LoadObjectsAndAbons(sPath, sSheet):
    """
    Загружает объекты и абонентов из Excel файла,
    проверяя и создавая всю иерархию объектов.
    """
    global cfg_excel_name, cfg_sheet_name
    cfg_excel_name = sPath
    cfg_sheet_name = sSheet
    result = "Объекты и абоненты не загружены."
    created_objects_count = 0
    created_abonents_count = 0

    dtAll = GetTableFromExcel(sPath, sSheet)
    ...
    for i in range(1, len(dtAll)):
        ...
        obj_l1 = get_or_create_object_with_parent(obj_l1_name, 1, parent=obj_l0)
        obj_l2 = get_or_create_object_with_parent(obj_l2_name, 2, parent=obj_l1)
        
        # Проверка и создание абонента
        try:
            Abonents.objects.get(name=abon_name, guid_objects=obj_l2)
        except Abonents.DoesNotExist:
            try:
                type_abonent = TypesAbonents.objects.get(guid="e4d813ca-e264-4579-ae15-385cdbf5d28c")
                abonent = Abonents(
                    name=abon_name,
                    account_1=account_1,
                    account_2=account_2,
                    guid_objects=obj_l2,
                    guid_types_abonents=type_abonent
                )
                abonent.save()
                created_abonents_count += 1
            except TypesAbonents.DoesNotExist:
                result = "Ошибка: Тип абонента с GUID e4d813ca-e264-4579-ae15-385cdbf5d28c не найден."
                return result

    if created_abonents_count > 0:
        result = f"Создано {created_abonents_count} новых абонентов."
    else:
        result = "Новые абоненты не были созданы (возможно, уже существуют)."
...
def LoadElectricMeters(sPath, sSheet):
    ...
    for i in range(1, len(dtAll)):
        try:
            ...
            adr = str(dtAll[i][7]).strip()
            type_meter = str(dtAll[i][8]).strip()
            NumLic = str(dtAll[i][5]).strip()
            ...
            # isNewAbon = False
            # if len(check_objects_and_abonents_structure(parent, obj, abon)) > 0:
            #     isNewAbon = True
            # isNewAbon = SimpleCheckIfExist('objects', 'name', obj_l2, 'abonents', 'name', abon)
            
            isR = (attr1 == '+')
            isHalfs = (attr2 == '+')
            is_ridan_impulse = (flag_impulse_ridan_col_o == 'да')

            # if not isNewAbon:
            #     errors.append(f"Абонент для счётчика '{meter}' (тип '{type_meter}') не найден. Сначала необходимо создать структуру объектов.")
            #     continue
            ...
            if not isNewMeter:
                ...
                if add_meter:
                    ...
                    print(f"Device added: {type_meter} - {meter}")
                    met += 1

        except TypesMeters.DoesNotExist:
            errors.append(f"Тип счётчика '{type_meter}' (зав. номер '{meter}') не найден в базе данных. Проверьте соответствие GUID в коде.")
        except Exception as e:
            errors.append(f"Неизвестная ошибка для счётчика '{meter}' (тип '{type_meter}'): {e}")

    result_list = [f"Загружено новых счётчиков: {met}."]
    if errors:
        result_list.append("Обнаружены ошибки:")
        result_list.extend(errors)
...
def service_water(request):
    args={}
    args['choice_file'] = request.session.get("choice_file", "")
    args['choice_sheet'] = request.session.get("choice_sheet", "")
...
#        #R+ Профиль
    ...
    # Суточные
      ...
      # Канал 20
        ...
        #Добавляем параметры для водосчётчика Ридан СГВ-15 ХВС.
        #------------Суточные
        # Объем_входящий, м3
        ...
        #-------------Суточные
        # ОбЪем 1 .
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "0df5bd35-8a69-47da-9e28-2d7f3b492926"))
        add_param.save()    
        # ОбЪем 2 .
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "50c593c1-ab75-47c8-9c5c-da13713fcaf0"))
        add_param.save()    
        # ОбЪем 3 .
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "554ecd96-18c4-4f9f-9cee-305764cbcbc5"))
        add_param.save()    
        # ОбЪем 4 .
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "3702fc3a-7a0a-4cad-8d1c-a702d950b4af"))
        add_param.save()    
        # ОбЪем 5 .
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "9656b53b-49a7-4ff9-8694-dc8a5d95a3dd"))
        add_param.save()    
        # ОбЪем 6 .
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "d1216f4e-35ad-4aa3-92dd-d5b8d576e9b7"))
        add_param.save()    
        # Энергия 1 .
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "75a548d8-04bf-48a0-a18a-f70c8d2aefbe"))
        add_param.save()    
        # Энергия 2 .
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "39b248ca-4928-4a69-958b-88c1b0aefa0f"))
        add_param.save()    
        # Энергия 3 .
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "fcbc58c0-631d-47b7-b200-a3a7036908f0"))
        add_param.save()    
        # Энергия 4 .
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "06fa9df2-c671-484d-a48a-18251f0a010f"))
        add_param.save()    
        # Температура 1 .
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "f6f28d56-84bd-4cbc-8173-3ed85460093c"))
        add_param.save()    
        # Температура 2 .
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "14645b7c-087d-4281-9bb8-8f7570311fb9"))
        add_param.save()    
        # Температура 3 .
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "8495f818-7c32-48fb-8c20-4283c20a4700"))
        add_param.save()    
        # Температура 4 .
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "9fb2b75e-904a-488a-9f83-7d79233c0bc9"))
        add_param.save()    
        # Температура 5 .
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "60001591-c938-4053-b3cc-91ae789dd715"))
        add_param.save()    
        # Температура 6 .
        ...
        #Добавляем параметры для ВЗЛЕТ МР УРСВ-311 ГВС.
        #------------Суточные
        # "Объём входящий", м3
        # print('!!!!!')
        ...
        #Добавляем параметры для ВЗЛЕТ МР УРСВ-311 ХВС.
        #------------Суточные
        # "Объём входящий", м3
    ...
    elif instance.guid_types_meters.name == 'CE308 СПОДЭС':
        #Добавляем параметры для счётчика CE308 СПОДЭС.    
        #-------------Суточные
        # "Показание". T0 A+
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "58d49e69-c779-4e5a-98bf-f598c833ce3e"))
        add_param.save()
        # "Показание". T1 A+
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "f27e1847-ff1c-417c-8e3e-4c653f3dc72e"))
        add_param.save() 
        # "Показание". T2 A+
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "22e995cc-a170-43b7-9cd4-e3da4ea28ace"))
        add_param.save() 
        # "Показание". T3 A+
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "dfbc97bc-1d20-491e-97ad-8dced70f1ac7"))
        add_param.save()
        ...
        #------------Часовые Система 2
        # "Показание Энергии" Q, Гкал
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "587da852-512a-461c-92c8-4dfb6f5df0ed"))
        add_param.save()
        # "Показание Расход воды" Объем, м3
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "1a53df66-e623-4571-be0c-32d6a85f6c11"))
        add_param.save()
        # "Показание Температура подачи" Ti, C0
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "191c4768-1cb3-450c-9d1a-fe3615c8f6b5"))
        add_param.save()
        # "Показание Температура выхода" To, C0
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "c4cbec74-3580-4b1e-bf23-34e9a1aa5844"))
        add_param.save()
        # "Gпод Суточный" Gi, тонны
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "f0b315df-213a-4522-abf2-9b76a9ca665d"))
        add_param.save()
        # "Gобр Суточный" Go, тонны  
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "0de7cff4-c459-413d-a9b3-c3a2e78f9087"))
        add_param.save()
        # "Pпод Суточный" Pi, атмосферы
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "b42181a3-e788-43aa-9b19-b40f920d84d9"))
        add_param.save()
        # "Pобр Суточный" Po, атмосферы 
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "ccfb4ca1-fa98-45da-98cb-6e8a33c2fdb8"))
        add_param.save()
        # "Время наработки" operating_hours, часы
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "e3e46248-5f52-4d18-a0b4-1cad42677c8e"))
        add_param.save()

        #------------Часовые Система 3
        # "Показание Энергии" Q, Гкал
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "03f473eb-29c8-4eea-8332-d08cebd3b553"))
        add_param.save()
        # "Показание Расход воды" Объем, м3
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "9ee3ff1d-c85a-4db1-a170-f86fb4ae4a91"))
        add_param.save()
        # "Показание Температура подачи" Ti, C0
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "6107f421-020f-4000-be29-9cbd8eb1ba3c"))
        add_param.save()
        # "Показание Температура выхода" To, C0
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "7a617b2d-40f8-4b9b-8f8b-e954d9437dac"))
        add_param.save()
        # "Gпод Суточный" Gi, тонны
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "a0fb81ca-d5f3-4d0f-81d6-e6f1e071637d"))
        add_param.save()
        # "Gобр Суточный" Go, тонны  
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "fffd11eb-1fa2-4cb3-abbb-49f9f4428719"))
        add_param.save()
        # "Pпод Суточный" Pi, атмосферы
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "dd109630-ac3d-4b0d-8b53-fd11d5ea7cb1"))
        add_param.save()
        # "Pобр Суточный" Po, атмосферы 
        add_param = TakenParams(id = TakenParams.objects.aggregate(Max('id'))['id__max']+1, guid_meters = instance, guid_params = Params.objects.get(guid = "811d9ade-f63b-4039-b469-7876676d00f5"))
        add_param.save()
        # "Время наработки" operating_hours, часы
...
def add_link_abonent_taken_params_from_excel_cfg_electric(sender, instance, created, **kwargs):
    ...
    for i in range(1,len(dtAll)):
        ...
        if meter is not None and str(meter) == instance.guid_meters.factory_number_manual:
            ...
            SELECT a.guid
            FROM abonents a
            JOIN objects obj2 ON a.guid_objects = obj2.guid
            JOIN objects obj1 ON obj2.guid_parent = obj1.guid
            JOIN objects obj0 ON obj1.guid_parent = obj0.guid
            WHERE a.name = %s
              AND obj2.name = %s
              AND obj1.name = %s
              AND obj0.name = %s
            """
            cursor.execute(sQuery, [abon, obj_l2_name, obj_l1_name, obj_l0_name])
            guid_abonent_by_excel = cursor.fetchall()

            if len(guid_abonent_by_excel) > 0:
                guidAbon =  guid_abonent_by_excel[0][0]
                print(f"{abon} - {instance.guid_meters.name}")
                common_sql.InsertInLinkAbonentsTakenParams(name = f"{abon} - {instance.guid_meters.name}", coefficient=str(dtAll[i][9]), coefficient_2 = 1, coefficient_3 = 1, guid_abonents = guidAbon, guid_taken_params = instance.guid)
            else:
                pass
...
def load_user_account(request):
    ...
    try:    
        if request.is_ajax():
            if request.method == 'GET':
                fileName = request.GET.get('choice_file')
                sheet = request.GET.get('choice_sheet')

                if not fileName or not sheet:
                    result = "Ошибка: Не выбран файл или лист для загрузки."
                else:
                    request.session["choice_file"] = fileName
                    request.session["choice_sheet"] = sheet
                    directory=os.path.join(BASE_DIR,'static/cfg/')
                    sPath=directory+fileName
                    try:
                        result = load_users_account(sPath, sheet)
                    except FileNotFoundError:
                        result = f"Ошибка: Файл '{fileName}' не найден на сервере."
                    except Exception as e:
                        result = f"Произошла непредвиденная ошибка: {e}"
    except Exception as e:
        result = f"Общая ошибка: {e}"
    ...
    try:
        if request.is_ajax():
            if request.method == 'GET':
                ...
                if not fileName or not sheet:
                    result.append("Ошибка: Не выбран файл или лист для загрузки.")
                else:
                    request.session["choice_file"] = fileName
                    request.session["choice_sheet"] = sheet
                    directory=os.path.join(BASE_DIR,'static/cfg/')
                    ...
                    try:
                        ...
                        result.append(f"Ошибка: Файл '{fileName}' не найден на сервере.")

    except Exception as e:
        result.append(f"Произошла непредвиденная ошибка: {e}")
    ...
    try:
        if request.is_ajax():
            if request.method == 'GET':
                ...
                sheet = request.GET.get('choice_sheet')
                if not fileName or not sheet:
                    result.append("Ошибка: Не выбран файл или лист для удаления.")
                else:
                    request.session["choice_file"] = fileName
                    request.session["choice_sheet"] = sheet
                    directory=os.path.join(BASE_DIR,'static/cfg/')
                    ...
                    try:
                        ...
                        result.append(f"Произошла непредвиденная ошибка при обработке файла: {e}")

    except Exception as e:
        result.append(f"Общая ошибка: {e}")
...
def load_digital_devices_sequentially(request):
    args={}
    fileName=""
    sheet    = ""
    sequential_results = []
    ...
    try:
        if request.is_ajax() and request.method == 'GET':
            ...
            if not fileName or not sheet:
                sequential_results.append("<li>Ошибка: Не выбран файл или лист.</li>")
            else:
                directory = os.path.join(BASE_DIR,'static/cfg/')
                sPath = directory + fileName
                ...
                # Step 1: Load Ports
                try:
                    ports_loaded = load_tcp_ip_or_com_ports_from_excel(sPath, sheet)
                    if ports_loaded:
                        sequential_results.append("<li>Шаг 1: Порты успешно загружены/проверены.</li>")
                    else:
                        sequential_results.append("<li>Шаг 1: Порты уже существуют в БД или не найдены в файле.</li>")
                except Exception as e:
                    sequential_results.append(f"<li>Шаг 1: Ошибка загрузки портов: {e}</li>")

                # Step 2: Load Objects and Abonents
                try:
                    object_status = LoadObjectsAndAbons(sPath, sheet)
                    sequential_results.append(f"<li>Шаг 2: {object_status}</li>")
                except Exception as e:
                    sequential_results.append(f"<li>Шаг 2: Ошибка загрузки объектов/абонентов: {e}</li>")
                ...
                try:
                    ...
                    isService=True
                    OnOffSignals()
                    counter_status = LoadElectricMeters(sPath, sheet)
                    
                    # Format the result from LoadElectricMeters
                    if isinstance(counter_status, list) and counter_status:
                        # Main message for step 3
                        step3_result_html = f"<li>Шаг 3: {counter_status[0]}"
                        # Check for nested errors
                        if len(counter_status) > 1:
                            step3_result_html += "<ul class='nested-result'>"
                            for item in counter_status[1:]:
                                step3_result_html += f"<li>{item}</li>"
                            step3_result_html += "</ul>"
                        step3_result_html += "</li>"
                        sequential_results.append(step3_result_html)
                    else:
                        sequential_results.append(f"<li>Шаг 3: {counter_status}</li>")

                    isService=False
                    OnOffSignals()
                except Exception as e:
                    sequential_results.append(f"<li>Шаг 3: Ошибка загрузки счётчиков: {e}</li>")

    except Exception as e:
        # General error
        sequential_results.append(f"<li>Общая ошибка: {e}</li>")
...
@csrf_exempt
def load_impulse_devices_sequentially(request):
    args={}
    fileName=""
    sheet    = ""
    sequential_results = []
    ...
    try:
        if request.is_ajax() and request.method == 'GET':
            ...
            if not fileName or not sheet:
                ...
                # Step 1: Load Ports
                try:
                    ports_loaded = load_tcp_ip_water_ports_from_excel(sPath, sheet)
                    if ports_loaded:
                        sequential_results.append("<li>Шаг 1: Порты успешно загружены/проверены.</li>")
                    else:
                        sequential_results.append("<li>Шаг 1: Порты уже существуют в БД или не найдены в файле.</li>")
                ...
                # Step 2: Load Objects and Abonents
                try:
                    object_status = LoadObjectsAndAbons_water(sPath, sheet)
                    sequential_results.append(f"<li>Шаг 2: {object_status}</li>")
                except Exception as e:
                    sequential_results.append(f"<li>Шаг 2: Ошибка загрузки объектов/абонентов: {e}</li>")

                # Step 3: Load Pulsar Devices
                try:
                    global isService
                    isService = True
                    OnOffSignals()
                    pulsar_status = LoadWaterPulsar(sPath, sheet)
                    sequential_results.append(f"<li>Шаг 3: {pulsar_status}</li>")
                    isService = False
                    OnOffSignals()
                except Exception as e:
                    sequential_results.append(f"<li>Шаг 3: Ошибка загрузки счётчиков Пульсар: {e}</li>")
